// ============================================================================
// Properties — resolve-or-create identity, and candidate lookup.
// ----------------------------------------------------------------------------
// A property is the DURABLE physical identity. The same building recurring in
// the pipeline in 2025, 2026 and 2027 is one property carrying three
// opportunities, which is what makes the timeline worth having.
//
// Before this module, createOpportunity() inserted a fresh `properties` row on
// every call, so the property-centric lifecycle documented in docs/10 was not
// actually true in the data. resolveProperty() is the fix, and every write path
// that creates a property must go through it.
//
// Every call runs inside the caller's transaction, so RLS decides the rows.
// ============================================================================
...
export interface ResolvedProperty {
  ...
  /** True when this call created the row; false when it matched an existing one. */
  created: boolean;
}
 ...
 *
 * Matching is by identity key alone — postcode plus street part, or the street
 * address where no postcode is given. A NULL key always creates and never
 * merges: a missed match becomes a review task, whereas a false match silently
 * fuses two buildings and corrupts the history of both.
 *
 * An existing row is ENRICHED, never overwritten: a blank column is filled and
 * a populated one is left alone. A later source that contradicts a recorded
 * value is a conflict to surface, not something to resolve by clobbering — the
 * same rule the investment case applies to money.
 */
export async function resolveProperty(
  ...
  if (identityKey) {
    ...
    if (existing.rows[0]) {
      ...
      return { propertyId, identityKey, created: false };
    }
  }
  ...
  const inserted = await tx.query<{ property_id: string }>(
    `insert into properties(org_id, name, address, postcode, city, country, country_code,
       market, submarket, asset_type, address_normalised, identity_key,
  ...
  return { propertyId: inserted.rows[0].property_id, identityKey, created: true };
}
...
/**
 * Plausible matches for a subject, narrowed in SQL.
 *
 * The database is good at "same postcode, or a similar address or name"; it is
 * not where a weighting belongs. This returns candidates and stops — scoring
 * and banding arrive with the matching layer, in one unit-tested place.
 */
export async function findCandidates(
  ...
  const { rows } = await tx.query<Record<string, unknown>>(
    `select p.property_id, p.name, p.address, p.postcode, p.city,
            ...
            o.opportunity_id, o.name as opportunity_name, o.stage, o.status, o.broker_name
       from properties p
       left join lateral (
         select o.opportunity_id, o.name, o.stage, o.status, o.broker_name
...
export async function getProperty(
  ...
  return withSession(session, async (tx) => {
    const { rows } = await tx.query(
      `select p.*, count(o.opportunity_id) as campaign_count
         from properties p
         left join opportunities o on o.property_id = p.property_id
        where p.property_id = $1
        group by p.property_id`, [propertyId]);
    if (!rows[0]) return null;
    return { ...rows[0], campaign_count: num(rows[0].campaign_count) };
  });
}
