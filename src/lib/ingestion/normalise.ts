 ...
 *
 * Where no street type is recognisable the fallback depends on whether a
 * postcode is carrying the key. Many London streets have no type suffix at all
 * - Bishopsgate, Cheapside, Aldwych, Poultry - and for those the second token
 * is the CITY, so "8 Bishopsgate, London" and "8 Bishopsgate" would key
 * differently and the same building would be recorded twice. With a postcode
